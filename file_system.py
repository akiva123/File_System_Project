def run_file_system():
            ...
            if type(value) == dict:
                if len(path) == 1 and current == path[0]:
                    if path_list1[0] in value.keys():
                        if type(value[path_list1[0]]) != dict:
                            return None
                        else:
                            if len(path_list1) == 1:
            ...
            if type(value) == dict:
                if len(path) == 1 and current == path[0] and key == path[0]:
                    if command in value.keys():
                        ...
                        value[command] = ''  # set value as 'file' to represent a file
                        return
                elif key == path[0]:
            ...
            if type(value) == dict:
                if len(path) == 1 and current == path[0] and key == path[0]:
                    if command not in value.keys():
                        ...
                        return
                    elif type(value[command]) == dict:  # if the desired entry is a file, don't attempt to enter it
                        print('not a file')
                        return
                    else:
                        del value[command]
    ...
    def rmdir(dictionary, path, command):
        current = return_current()
        for key, value in dictionary.items():
            if type(value) == dict:
                if len(path) == 1 and current == path[0] and key == path[0]:
                    ...
                    return rmdir(value, path[1:], command)

    def locate(dictionary, path, search, condition=False, path_string=''):
        current = return_current()
        for key, value in dictionary.items():
            if type(value) == str and condition == True:
                if search in key:
                    print('/' + path_string + key)
            if len(path) == 1 and type(value) == dict:
                if path_string == '' and key == 'root':
                    path_string += ''
                else:
                    path_string += f'{key}/'
                locate(value, path, search, condition=True,
                       path_string=path_string)  # don't use return because want to iterate over all key value pairs
                path_string = '/'.join(path_string.split('/')[:-2]) + '/'
            elif key == path[0]:
                return locate(value, path[1:], search, condition, path_string='')

    def nano(dictionary, path, file):
        current = return_current()
        for key, value in dictionary.items():
            if type(value) == dict:
                if len(path) == 1 and current == path[0] and key == path[0]:
                    if file in value.keys():
                        if type(value[file]) == str:
                            val = None
                            while val != 'exit':
                                print(value[file])
                                val = input(
                                    'type "replace" to replace text, "append" to append to the text, or "exit" to exit nano: ')
                                if val == 'replace':
                                    replace = input('what do you want to replace? ')
                                    replace_with = input('what do you want to replace it with? ')
                                    value[file] = value[file].replace(replace, replace_with)
                                    print()
                                elif val == 'append':
                                    append = input('what do you want to append? ')
                                    value[file] = value[file] + append
                                    print()
                                elif val == 'exit':
                                    print()
                                else:
    ...
    while command != 'exit':
        ...
        if command == 'ls':
            ...
            if len(command.split(' ')) != 2:
                ...
                rmdir(dictionary, direct_path.split('/')[1:-1], command[6:])
            ...
            if len(command.split(' ')) != 2:
                ...
                nano(dictionary, direct_path.split('/')[1:-1], command[5:])
